...
import { SearchOutlined, DownloadOutlined, EyeOutlined, EditOutlined, FilterOutlined } from "@ant-design/icons";
import dayjs from "dayjs";

const reasonsList = ["Quality Issue", "Damaged Packaging", "Expired", "Wrong Item"];
  ...
  records: [
    {
      ...
      items: [
        ...
        { id: "it-2", item: "Mustard Oil", itemCode: "code2", uom: "Ltr", rate: 600, quantity: 20, returnQty: 5, otherReasonText: "Xyz", returnReason: "Other" },
      ],
    },
    {
      key: 2,
...
export default function RaiseDispute() {
  ...
  useEffect(() => {
    ...
    setFilteredData(
      records.filter(r =>
        `${r.invoiceNo} ${r.orderNo} ${r.plantName}`.toLowerCase().includes(val)
      )
    );
  }, [searchText, records]);
  ...
  const openModal = (record, mode = "view") => {
    ...
    if (mode !== "view") {
      const items = (record.itemsReturned || record.items || []).map(it => ({
        ...it,
  ...
  const handleSave = async () => {
    try {
      ...
      // Validate at least one item has returnQty > 0
      const hasReturn = updatedItems.some(i => i.returnQty > 0);
      if (!hasReturn) {
        Modal.error({ title: "Validation Error", content: "Please enter return quantity for at least one item." });
        return;
      }

      // Validate reason for items with returnQty
      const invalidReason = updatedItems.some(
        i => i.returnQty > 0 && (!i.returnReason || (i.returnReason === "Other" && !i.otherReasonText))
      );
      if (invalidReason) {
        Modal.error({ title: "Validation Error", content: "Please provide a reason for all items with return quantity." });
        return;
      }
  ...
  const statusTag = status => {
    const map = { Approved: "bg-green-100 text-green-700", Pending: "bg-yellow-100 text-yellow-700", Delivered: "bg-red-100 text-red-700" };
    return <span className={`px-3 py-1 rounded-full text-sm font-semibold ${map[status]}`}>{status}</span>;
  };

  const columns = [
    { title: <span className="text-amber-700!">Invoice No</span>, dataIndex: "invoiceNo" ,render: (text) => <span className="text-amber-700!">{text}</span>},
    { title: <span className="text-amber-700!">Order No</span>, dataIndex: "orderNo", render: (text) => <span className="text-amber-700!">{text}</span> },
    { title: <span className="text-amber-700!">Return Date</span>, dataIndex: "returnDate" ,render: (text) => <span className="text-amber-700!">{text}</span>},
    { title: <span className="text-amber-700!">Plant</span>, dataIndex: "plantName", render: (text) => <span className="text-amber-700!">{text}</span> },
    { title: <span className="text-amber-700!">Items</span>, render: (_, r) => <span className="text-amber-700!">{(r.itemsReturned || r.items || []).map(x => x.item).join(", ")}</span> },
    { title: <span className="text-amber-700!">Status</span>, dataIndex: "status", render: statusTag },
    {
      title: <span className="text-amber-700!">Actions</span>,
      width: 150,
      render: record => (
        <div className="flex gap-3 items-center">
          <EyeOutlined className="cursor-pointer! text-blue-500!" onClick={() => openModal(record, "view")} />
          {record.status === "Pending" && (
            <EditOutlined className="cursor-pointer! text-orange-500!" onClick={() => openModal(record, "edit")} />
          )}
  ...
  const modalColumns = [
    { title: <span className="text-amber-700!">Item</span>, dataIndex: "item" },
    { title: <span className="text-amber-700!">Item Code</span>, dataIndex: "itemCode" },
    { title: <span className="text-amber-700!">UOM</span>, dataIndex: "uom" },
    { title: <span className="text-amber-700!">Rate</span>, dataIndex: "rate" },
    { title: <span className="text-amber-700!">Order Qty</span>, dataIndex: "quantity" },
    {
    ...
    },
    {
      ...
      render: (_, row) => {
        if (modalMode === "view") {
          return row.returnReason === "Other" ? row.otherReasonText || "" : row.returnReason;
        } else {
          return (
            <Form.Item name={`reason_${row.id}`} style={{ margin: 0 }}>
              ...
              >
                {reasonsList.map(r => <Select.Option key={r}>{r}</Select.Option>)}
                <Select.Option value="Other">Other</Select.Option>
              </Select>
              {isOtherReason[row.id] && (
                <Form.Item name={`other_${row.id}`} style={{ marginTop: 5 }}>
                  <Input placeholder="Other reason" value={form.getFieldValue(`other_${row.id}`)} onChange={e => form.setFieldsValue({ [`other_${row.id}`]: e.target.value })} />
                </Form.Item>
              )}
            </Form.Item>
          );
    ...
    },
    {
      ...
      render: (_, row) => {
        const qty = modalMode === "view" ? row.returnQty || 0 : qtyState[row.id] || 0;
  ...
  return (
    <div>
      ...
      <Modal
        title={modalMode === "view" ? "View Dispute" : modalMode === "edit" ? "Edit Items" : "Dispute Items"}
        open={isModalOpen}
      ...
      >
        {modalRecord && (
          <Form form={form} layout="vertical">
            <Row gutter={16}>
              <Col span={6}><Form.Item label="Invoice No"><Input value={modalRecord.invoiceNo} disabled /></Form.Item></Col>
              <Col span={6}><Form.Item label="Order No"><Input value={modalRecord.orderNo} disabled /></Form.Item></Col>
              <Col span={6}><Form.Item label="Plant"><Input value={modalRecord.plantName} disabled /></Form.Item></Col>
              <Col span={6}><Form.Item label="Date"><Input value={dayjs().format("YYYY-MM-DD")} disabled /></Form.Item></Col>
            </Row>
